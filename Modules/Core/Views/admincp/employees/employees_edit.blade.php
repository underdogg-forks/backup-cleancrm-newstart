...
@section('content')
    ...
    <form method="post" action="{{ route('employees.update', ['id' => $employee->id]) }}" data-parsley-validate class="form-horizontal form-label-left">
        ...
        <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }} row">
            ...
            <div class="col-sm-10">
                <input type="text" value="{{$employee->name}}" id="name" name="name" class="form-control col-md-7 col-xs-12"> @if ($errors->has('name'))
        ...
        <div class="form-group{{ $errors->has('email') ? ' has-error' : '' }} row">
            ...
            <div class="col-sm-10">
                <input type="text" value="{{$employee->email}}" id="email" name="email" class="form-control col-md-7 col-xs-12"> @if ($errors->has('email'))
        ...
        <div class="form-group{{ $errors->has('role_id') ? ' has-error' : '' }} row">
            ...
            <div class="col-md-6 col-sm-6 col-xs-12">
                <select class="form-control" id="role_id" name="role_id">
                    ...
                    <option value="{{$row->id}}" {{$row->id == $employee->roles[0]->id ? 'selected="selected"' : ''}}>{{$row->name}}</option>
                    @endforeach
                    @endif
                </select>
